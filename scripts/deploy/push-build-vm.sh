...
ssh -F /dev/null -i "$KEY" -o StrictHostKeyChecking=no "$BUILD_VM" "sudo rm -rf $REMOTE && sudo mkdir -p $REMOTE/bin/arm64 $REMOTE/docs/deploy $REMOTE/scripts/deploy"
...
ssh -F /dev/null -i "$KEY" -o StrictHostKeyChecking=no "$BUILD_VM" "
